...
import (
	...
	httpgw "github.com/varwof/gateway/http"
)
...
func main() {
	...
	case "--version", "-v", "version":
		fmt.Println(httpgw.VersionString())
	case "--help", "-h", "help":
		usage()
	default:
		runServer(os.Args[1:])
...
Listener key=value format:
  name=<name>,listen=<addr>,tls-mode=<mode>,
...
Examples:
  ...
  gateway-http -L name=api,listen=:443,tls-mode=mtls,ca-cert=ca.pem,cert=cert.pem,key=key.pem -r listener=api,path=/api/v1,target=be:8080,allow-roles=gateway:admin -r listener=api,path=/,target=web:3000
`, httpgw.DefaultConfigFile())
}

func runServer(args []string) {
	fs := flag.NewFlagSet("server", flag.ExitOnError)
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelInfo}))
	cfgDefault := httpgw.DefaultConfigFile()
	...
	bundle := httpgw.NewBundle()
	lang := httpgw.DetectLang(*langFlag, "", os.Getenv("LANG"))
	...
	var cfg *httpgw.Config
	var err error

	if hasCLI {
		cfg, err = httpgw.BuildConfigFromCLI(listeners.values, routes.values, &httpgw.CLIGlobals{
		...
		cfg, err = httpgw.LoadConfig(*configPath)
		if err != nil {
	...
	for _, l := range cfg.Listeners {
		...
		if tsaClient == nil {
			...
			// W35 (2026-08-16): TSA is a gateway-level shared service; subsequent listeners'
			// tsa_url is ignored — consistent with audit_file shared warning, avoiding silence.
	...
	g := httpgw.NewGateway(cfg, bundle, lang, auditLogger, tsaClient, tsaProofLogger, logger)
