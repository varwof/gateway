...
func TestBundle_T(t *testing.T) {
	...
	if got != "gateway-http server started" {
		t.Fatalf("expected 'gateway-http server started', got %q", got)
	}

	got = b.T("zh", "gateway.started")
	if got != "gateway-http 服务已启动" {
		t.Fatalf("expected 'gateway-http 服务已启动', got %q", got)
	}
}
...
func TestBundle_Ef(t *testing.T) {
	...
	err := b.Ef("en", "gateway.started")
	if err == nil || err.Error() != "gateway-http server started" {
		t.Fatalf("Ef(en, gateway.started) = %v", err)
	}

	err = b.Ef("en", "nonexistent.key")
	if err == nil || err.Error() != "nonexistent.key" {
		t.Fatalf("Ef(en, missing key) = %v", err)
	}

	var nb *Bundle
	err = nb.Ef("en", "gateway.started")
	if err == nil {
		t.Fatal("Ef on nil Bundle should still return an error")
	}
}
